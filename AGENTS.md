...
- **Never edit frozen binaries**: `payload.bin` (GoldHEN, redistributed
  unmodified), `patches/*.bin` (kernel patch blobs), `logo_raw.png`.
- **Never edit the third-party SLOPKIT sources**: `core.js`, `mem.js`,
  `int64.js`, `rpc_worker.js`. Keep them unmodified to preserve the licensing
  boundary; changes there belong upstream or in the chain files.
...
- **Keep `cache.appcache` in sync**: it pins a lowercase SHA-256 hex digest
  ...
  - `core.js` has **two entries** - `core.js` and `core.js?v=10` - and both
    must always carry the same hash. Update them together.
...
run_lapse.html / run_poops.html          (thin shells, type="module")
  |
  v
chain_lapse.js / chain_poops.js          (linear stages 1..10, gated + logged)
...
- Post-exploitation order is fixed: root escalation -> sandbox escape
  ...
  verifying (`chain_lapse.js:207-253` loads the blobs; the payload only runs
...
- **64-bit arithmetic is mandatory.** Every pointer/value above 32 bits is a
  `new int64(lo, hi)` (`int64.js`) with `>>> 0` discipline on both words.
  Never pass a plain `Number` wider than 32 bits where a 64-bit value is
  expected. `mem.js` normalizes numeric/object addresses and rejects
  non-canonical ones (`hi > 0xffff`) - keep that validation (`mem.js:26-30`).
- **Use the published memory API.** `installWindowP()` publishes `window.p`
  with `read1/2/4/8`, `write1/2/4/8` and `leakval` (`mem.js:733-743`). Wrap
  every carrier access in `aim ... finally restore` so a throw cannot leave
  the window mis-aimed. If a promotion fails and its rollback cannot verify,
  `window.p` is withdrawn by design (`mem.js:757-762`) - never re-publish a
  broken primitive.
- **Keep module specifiers consistent.** `mem.js:6` and `chain_poops.js:4`
  import `./core.js?v=10` on purpose: the query string is part of the module
  identity, and a mismatched specifier builds a second `core.js` instance -
  the exact failure documented at `chain_poops.js:1-3` (pins ~137 MB).
  Use the same specifier everywhere; if you change it, update the matching
  `cache.appcache` entry too. `chain_lapse.js:1` currently imports
  `./core.js` without the query - see section 7.
...
- **Keep log tags stable.** `mark()` tags are the contract used to read
  hardware logs; never rename or repurpose an existing tag. Keep `terse()`
  compact output intact and add detail behind `?verbose=1`.
- **Error handling.** Annotate failures with named tags (for example
  ...
  (`chain_lapse.js:3785-3815`): a dirty teardown must say so explicitly.
...
## 5. Firmware & Offsets Rules (`ps4_offsets.js`)

- `ps4_offsets.js` is the single source of truth for WebKit gadget offsets
  and kernel RVAs. Never hardcode offsets in chain files; read them through
  `offsetsFor()` (`ps4_offsets.js:447`), keyed off the User-Agent.
...
- Keep the `REQUIRED_KEYS` and `OPTIONAL_KEYS` lists (`ps4_offsets.js:1-27`)
  in sync with the keys you add or remove.
...
- **README vs `fw_status`**: the README support table claims hardware tests
  for all seven firmwares, while `ps4_offsets.js` marks 12.00, 12.02, 12.50
  and 12.52 as `UNTESTED-on-hardware`, and project history confirms only
  11.00, 11.50 and 13.00 were confirmed end-to-end. Trust `fw_status` and
  hardware logs; fix the README when evidence arrives, not with assumptions.
- **`chain_lapse.js` core.js import**: line 1 imports `./core.js` while
  `mem.js:6` and `chain_poops.js:4` import `./core.js?v=10`. This is the
  module-duplication pattern the poops comment warns about. Treat any change
  here as a deliberate fix, verify on hardware, and update both
  `cache.appcache` entries if the specifier changes.
- **Missing tools**: `tools/checkfw.js` and `tools/addfw.js` are referenced in
  `ps4_offsets.js` comments but do not exist in the repository. Do not assume
  they are available; ask before creating them.
- **Diagnostic beacon**: `post()` sends an XHR to the relative path `"t"`.
  There is no such file in the repo; a 404 there is expected and is not a
  failure.
